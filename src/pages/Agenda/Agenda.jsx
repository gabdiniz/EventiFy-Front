...
export function Agenda() {
  ...
  async function listarTalks() {
    await axios
      .get(`${process.env.REACT_APP_IP}/talks?eventId=${id}`, {
  ...
  async function listarEvents() {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_IP}/events?id=${id}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
  ...
  async function listarEventSpeaker() {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_IP}/eventspeakers?eventId=${id}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
  ...
  async function listarRegistration() {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_IP}/registrations/event/${id}/user/${userId}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
      ...
      <div className="agenda">
        <Container className="container-agenda">
          {loader ? (
                  ...
                  return (
                    <Tab
                      ...
                      {filteredTalks.map((talk) => {
                        ...
                        const horaInicial =
                          dataConvertidaInicial.toLocaleTimeString("pt-br", {
                            hour: "2-digit",
                            minute: "2-digit",
                          });
                        const horaFinal =
                          dataConvertidaFinal.toLocaleTimeString("pt-br", {
                            hour: "2-digit",
                            minute: "2-digit",
                          });
                        const info = talk.id === mostrarInfo;
                        const speakers = eventSpeaker.filter(
                          (speaker) => speaker.speakerId === talk.speakerId
                        );
                        console.log(speakers);
                        return (
                          <Card
                            key={talk.id}
                            className="d-flex justify-content-center gap-4 card-agenda"
                            onClick={() => handleCardClick(talk.id)}>
                            <p>{talk.name}</p>
                                        ...
                                        <div className="card-info-agenda">
                                          <div className="card-info-agenda-speaker">
                                            ...
                                            <div className="info-speaker-agenda">
                                              ...
                                              <span id="span-description-agenda">
                                                {speaker.speaker.description}
                                              </span>
                                            </div>
                                          </div>
                                          <span id="span-speaker">
                                            <small>
            ...
            emptyAgenda && (
              <h4>
                <small>Ainda não há programação para esse evento.</small>
              </h4>
            )
          )}
        </Container>
